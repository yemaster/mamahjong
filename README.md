...
`Tab` 选择规则，按方向键或 `Space` 修改。

常用按键：

- 登录/注册：`F2`、`Tab`、`Enter`
- 大厅：`4` 四麻匹配、`3` 三麻匹配、`n` 建房、方向键选房、`Enter` 加入
- 房间：`Space` 准备、`s` 开始
- 对局：方向键选牌，`Enter` 打牌，`r` 立直，`t` 自摸，`h` 荣和
- 副露：`c` 吃，`p` 碰，`k` 杠，`s` 过；有多种组合时用 `Space` 标记手牌
- 终局：`Esc` 返回房间

弃牌后无人可响应时会直接进入下一家摸牌。只有底部操作栏出现吃、碰、杠、
荣和或过时才需要选择；唯一可用的副露组合会直接采用。
