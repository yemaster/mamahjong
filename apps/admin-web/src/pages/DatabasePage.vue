...
const summaries = computed(() => [
  { label: "存储引擎", value: resource.data.value?.engine ?? "-" },
  { label: "连接状态", value: resource.data.value?.status ?? "-" },
  { label: "记录总数", value: total.value },
]);
</script>

<template>
  <PageShell title="数据概览" :error="resource.error.value" :loading="resource.loading.value">
    <template #actions><Button icon="pi pi-refresh" severity="secondary" variant="text" aria-label="刷新" :loading="resource.loading.value" @click="resource.reload" /></template>
    <div class="grid">
      <div v-for="item in summaries" :key="item.label" class="col-12 md:col-4"><Card class="h-full"><template #content><div class="text-color-secondary mb-2">{{ item.label }}</div><div class="text-2xl font-semibold">{{ item.value }}</div></template></Card></div>
    </div>
