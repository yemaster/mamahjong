...
async function remove(ids: string[]) {
  ...
  const success = await actions.run(() => Promise.all(ids.map((id) => adminApi.deleteCharacter(id, csrf))), "角色已删除");
...
<template>
  <PageShell title="角色" :error="pageError" :loading="resource.loading.value">
    <template #actions><Button icon="pi pi-refresh" severity="secondary" variant="outlined" aria-label="刷新" :loading="resource.loading.value" @click="resource.reload" /><Button label="添加角色" icon="pi pi-plus" @click="router.push({ name: 'character-new' })" /></template>
    <DataTable v-model:selection="selected" :value="rows" data-key="id" paginator :rows="10" :rows-per-page-options="[10, 20, 50]" scrollable table-style="min-width: 64rem">
        <template #header><div class="flex align-items-center justify-content-between gap-3 flex-wrap"><span>全部角色（{{ rows.length }}）</span><div class="flex gap-2 flex-wrap"><IconField><InputIcon class="pi pi-search" /><InputText v-model="search" placeholder="搜索名称或编号" /></IconField><FileUpload mode="basic" accept=".json,application/json" :max-file-size="5242880" auto custom-upload choose-label="导入" choose-icon="pi pi-upload" :disabled="actions.pending.value" @uploader="importCharacters" /><Button label="导出所选" icon="pi pi-download" severity="secondary" variant="outlined" :disabled="!selected.length" @click="exportSelected" /><Button v-if="selected.length" :label="`删除所选（${selected.filter((item) => !item.is_default).length}）`" icon="pi pi-trash" severity="danger" variant="outlined" @click="confirmRemove($event, selected)" /></div></div></template>
