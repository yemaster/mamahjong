...
曲目本体放在 `apps/game-web/public/assets/local-game-assets/music/`（随本地素材
一起不入 git），曲目信息写在数据库表 `mamahjong_music_tracks` 里：曲名、场景
（`lobby` / `match`）、路径、时长、是否可用、是否默认。每个场景各有一首默认曲，
由该表上的部分唯一索引保证。首次连库时通过 migration `0008_seed_music_tracks.sql` 播种。

玩家选的曲目记在 `mamahjong_users` 的 `selected_lobby_music_id` /
`selected_match_music_id` 上，`GET /api/v1/music-tracks` 取曲库，
`PUT /api/v1/users/me/music` 改选择。

客户端的加载约定：

- 进大厅之前，加载画面必须把大厅曲load完；进对局之前，云雾必须把对局曲load完。
- 曲子load完就发 `game.assets_ready`。四家都报到之前，服务端拒收一切对局命令，
  牌局也不开始；本家load完而别家没好时，云雾中间改显示「等待其他玩家(x/y)」。
- 某一家迟迟不报到，`MATCH_ASSET_LOAD_TIMEOUT_MS`（60 秒）到期后整局作废，各家
  收到「有玩家出现网络问题，对局已终止」并回房间。
- 素材缺失或解码失败一律按「load完」处理（见上面的占位行为），不能把玩家卡在
  加载里。
