...
可玩版在开局、每局结算和整场结束时原子更新一份 `match_record.v1` JSON。
Docker 使用持久命名卷，本地运行默认使用 `data/`。记录保留规则快照、玩家
快照、逐局点数与结果、该局完整领域事件以及整场顺位。进行中小局仍只在
内存；已结算小局的事件已经进入归档。格式与迁移边界见
[ADR-0009](adr/0009-mvp-record-archive.md)。

账号和运行中状态目前仍在内存中。下述 PostgreSQL 设计仍是目标
架构，不把 JSON 归档扩展成临时数据库。
